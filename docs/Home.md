...
[`StoneScriptAPI`](https://github.com/TweeZee/ssrpg-bun/blob/main/src/schema/api.ts), a type-level model of the game's own API
...
Output is Markdown, shaped for a GitHub wiki by `typedoc-plugin-markdown` and
...
MIT. See [LICENSE](https://github.com/TweeZee/ssrpg-bun/blob/main/LICENSE) — original copyright ArtificialPotato and contributors.
